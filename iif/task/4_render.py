...
class MitsubaRenderChristmas(Task):
    TASK_NAME = "render"
    MODALITY_TO_EXTENSION = {
        "rgbs_ldr": ".png",
        "rgbs_hdr": ".exr",
    ...
    def render_single(self, 
        ...
        # Update the scene params
        params = mitsuba.traverse(self.scene)
        self.module_logger.info(params)
        self.module_logger.info(f"Control points: {params['christmas_0_curve.control_points'].torch().view(4,4).tolist()}")
        params['camera.to_world'] = camera_transform
    ...
    def prepare_scene(self):
        ...
        def create_christmas_lights(name_prefix, translation, num_lights=10):
            """
            Create a B-spline curve with 4 emitter spheres along it.
            
            Parameters
            ----------
            name_prefix : str
                Unique prefix so shapes/emitter names don't collide.
            start, end : tuple of float
                Start and end 3D positions (x, y, z). The curve will be elevated to `height`.
            height : float
                Height (y or z depending on your coordinate system) at which the curve should run.
            sphere_radius : float
                Radius of each emitter sphere.
                
            Returns
            -------
            dict
                Mitsuba-3 scene dictionary entries for the curve + four spheres.
            """
            ...
            curve = {
                f"{name_prefix}_curve": {
                    "type": "bsplinecurve",
                    "filename": "/cluster/hithlum/pkocsis/projects/priorbasedinverserendering/configs/task/4_render/cable.txt",

                    "to_world": mitsuba.ScalarTransform4f().translate(translation),
            ...
            # Sphere positions at t = 0.1, 0.35, 0.65, 0.9 along the BSpline
            ts = [0.125, 0.375, 0.625, 0.875]
            cps = control_points  # For evaluation

            def eval_bspline(t):
                # Catmull-Rom-like B-spline evaluation (simple cubic uniform BSpline)
                # Basis:
                B0 = (1 - t)**3 / 6
                B1 = (3*t**3 - 6*t**2 + 4) / 6
                B2 = (-3*t**3 + 3*t**2 + 3*t + 1) / 6
                B3 = t**3 / 6
                return B0*cps[0] + B1*cps[1] + B2*cps[2] + B3*cps[3]
            ...
            for light_idx in range(num_lights):
                for t_idx, t in enumerate(ts):
                    ...
                    current_t = light_idx / num_lights + t / num_lights

                    pos = eval_bspline(current_t)
                    pos[2] -= 0.02  # Slightly below the cable

                    color = colors[t_idx]

                    spheres[f"{name_prefix}_light_{light_idx}_{t_idx}"] = {
                        "type": "sphere",
                        "center": list(np.array(pos) + np_translation),
                        "radius": 0.01,
                        ...
                        "emitter": {
                            ...
                            "radiance": {
                                "type": "rgb",
                                "value": list(np.array(color) * 400)
                            }
                        },

                        # Slightly glossy to look like bulbs
                        "bsdf": {
                            "type": "diffuse",
                            "reflectance": {"type": "rgb", "value": [0.0, 0.0, 0.0]}
                        }
        ...
        for idx in range(1):
            christmas_lights = create_christmas_lights(
                name_prefix=f"christmas_{idx}",
                translation=[3.0, 3.0, 5.]
            )
            scene_dict.update(christmas_lights)
