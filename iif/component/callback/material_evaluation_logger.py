...
class MaterialEvaluationLogger(ScheduledCallback):
    """
    ...
    def __call__(self, trainer, pl_module, outputs, batch, *args, **kwargs):
        ...
        for batch in samples.dataloader:
            ...
            with trainer.strategy.precision_plugin.train_step_context():
                # Run the model
                out = pl_module.training_step(batch, **self.forward_cfg)
        ...
        if self.transform is not None:
            eval_outputs = self.transform(eval_outputs)

        # Log the required data
        outputs[self.context] = eval_outputs
